...
namespace Miscellaneous
{
    ...
    {
        lua_pushstring(L, "NotZayduck");
        lua_pushstring(L, "1.67");
        return 2;
    }
    ...
    inline int filtergc(lua_State* L)
    {
        ...
        if (isFunction) {
            lua_getfield(L, optionsTable, "Name");
            if (lua_isstring(L, -1)) filterName = lua_tostring(L, -1);
            lua_pop(L, 1);

            lua_getfield(L, optionsTable, "IgnoreExecutor");
            if (lua_isboolean(L, -1)) ignoreExecutor = lua_toboolean(L, -1);
            lua_pop(L, 1);

            lua_getfield(L, optionsTable, "Hash");
            if (lua_isstring(L, -1)) filterHash = lua_tostring(L, -1);
            lua_pop(L, 1);

            lua_getfield(L, optionsTable, "Constants");
            if (lua_istable(L, -1)) constantsRef = lua_ref(L, -1);
            lua_pop(L, 1);

            lua_getfield(L, optionsTable, "Upvalues");
            if (lua_istable(L, -1)) upvaluesRef = lua_ref(L, -1);
            lua_pop(L, 1);
        }
        else {
            lua_getfield(L, optionsTable, "Keys");
            if (lua_istable(L, -1)) keysRef = lua_ref(L, -1);
            lua_pop(L, 1);

            lua_getfield(L, optionsTable, "Values");
            if (lua_istable(L, -1)) valuesRef = lua_ref(L, -1);
            lua_pop(L, 1);

            lua_getfield(L, optionsTable, "KeyValuePairs");
            if (lua_istable(L, -1)) kvpRef = lua_ref(L, -1);
            lua_pop(L, 1);

            lua_getfield(L, optionsTable, "Metatable");
            if (lua_istable(L, -1)) metatableRef = lua_ref(L, -1);
            lua_pop(L, 1);
        }
        ...
        while (lua_next(L, gcTableIdx) != 0) {
            ...
            if (isFunction && lua_isfunction(L, objIdx)) {
                ...
                if (filterName && matches) {
                    ...
                    if (cl->isC) {
                        if (cl->c.debugname) {
                            funcName = cl->c.debugname;
                        }
                    }
                    else if (cl->l.p && cl->l.p->debugname) {
                        TString* ts = cl->l.p->debugname;
                        if (ts) {
                            funcName = getstr(ts);
                        }
                    }

                    if (!funcName || strcmp(funcName, filterName) != 0) {
                        matches = false;
                    }
                }
